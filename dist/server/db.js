...
function createBoardColumns(boardId) {
...
export function getOrCreateProjectBoard(projectPath) {
    const resolved = path.resolve(projectPath);
    const existing = db.prepare("SELECT * FROM boards WHERE project_path = ?").get(resolved);
    if (existing)
        return existing;

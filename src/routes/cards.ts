...
cardsRouter.post("/", (req, res) => {
  ...
  const maxPos = db.prepare("SELECT MAX(position) as max FROM cards WHERE column_id = ?").get(column_id) as { max: number | null };
...
cardsRouter.post("/:id/move", (req, res) => {
  ...
  if (targetPos === undefined) {
    const maxPos = db.prepare("SELECT MAX(position) as max FROM cards WHERE column_id = ?").get(targetColId) as { max: number | null };

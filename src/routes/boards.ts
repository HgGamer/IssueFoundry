...
boardsRouter.get("/:id/full", (req, res) => {
  const board = db.prepare("SELECT * FROM boards WHERE id = ?").get(req.params.id) as Record<string, unknown> | undefined;
  if (!board) { res.status(404).json({ error: "Not found" }); return; }
  const columns = db.prepare("SELECT * FROM columns WHERE board_id = ? ORDER BY position").all(req.params.id) as Array<Record<string, unknown>>;
